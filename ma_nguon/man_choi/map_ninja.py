...
class MapNinjaScene(BaseMapScene):
    def __init__(self, game, player=None):
        ...
        self.parallax_bg.add_layer("tai_nguyen/hinh_anh/canh_nen/mapninjja/bau_troi.png", speed_factor=0.05, y_pos=0)
        
        # Lớp 2: Mây (di chuyển rất chậm)
        self.parallax_bg.add_layer("tai_nguyen/hinh_anh/canh_nen/mapninjja/may.png", speed_factor=0.1, y_pos=50)

        # Lớp 3: Núi xa (di chuyển chậm)
        self.parallax_bg.add_layer("tai_nguyen/hinh_anh/canh_nen/mapninjja/nui.png", speed_factor=0.2, y_pos=10, scale_factor=1.2)

        # Lớp 4: Mặt đất (di chuyển cùng tốc độ camera)
        self.parallax_bg.add_layer("tai_nguyen/hinh_anh/canh_nen/mapninjja/mat_dat.png", speed_factor=1.0, y_pos=230, repeat_x=True)
        ...
        for group in range(4):
            ...
            for i in range(num_enemies):
                ...
                enemy = QuaiVat(x_pos, 300, folder_qv, sound_qv, color=(255, 165, 0), damage=12)
                # Thiết lập vùng hoạt động phù hợp
                enemy.speed = 2.5
                enemy.home_x = x_pos
                enemy.patrol_range = 250
                enemy.aggro_range = 350
        ...
        # Điều chỉnh sức mạnh boss phù hợp với mùa thu
        for boss in self.bosses:
            boss.hp = int(boss.hp * 1.2)  # 20% máu hơn
            boss.damage = int(boss.damage * 1.1)  # 10% sát thương hơn
            boss.speed = boss.speed * 1.1  # 10% tốc độ hơn
    ...
    def spawn_next_boss(self):
        ...
        if self.current_boss_index < len(self.bosses):
            ...
            print(f"[MAP_NINJA] Spawned boss {self.current_boss_index}/{len(self.bosses)}")
        else:
            self.current_boss = None
            # Kiểm tra xem đã tiêu diệt tất cả kẻ địch chưa
            if not self.normal_enemies:
                # Chuyển sang màn tiếp theo
                print("[MAP_NINJA] All enemies defeated! Victory!")
    ...
    def update(self):
        ...
        if self.player.hp > 0:
            ...
            if current_time - self.death_timer > 2000:
                ...
                self.game.game_over_scene = self.game.load_scene("game_over", "Map ninja", score)
    ...
    def draw(self, screen):
        ...
        text = self.font.render("Map Ninja: Đại chiến ninja", True, (139, 69, 19))
