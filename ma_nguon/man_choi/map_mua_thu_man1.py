...
class MapMuaThuMan1Scene(BaseMapScene):
    def __init__(self, game, player=None):
        ...
        for group in range(2):
            ...
            for i in range(num_enemies):
                ...
                enemy = QuaiVat(x_pos, 300, folder_qv, sound_qv, color=(255, 200, 100), damage=15)  # Tăng từ 8 lên 15
                # Thiết lập vùng hoạt động - tăng sức mạnh
                enemy.hp = int(enemy.hp * 1.0)  # Giữ nguyên HP thay vì giảm
                enemy.speed = 2.5  # Tăng từ 2 lên 2.5
                enemy.home_x = x_pos
                enemy.patrol_range = 200  # Tăng từ 150 lên 200
                enemy.aggro_range = 350  # Tăng từ 250 lên 350
        ...
        # Boss mạnh hơn cho màn 1
        for boss in self.bosses:
            boss.hp = int(boss.hp * 1.0)  # Tăng từ 0.7 lên 1.0 (HP chuẩn)
            boss.damage = int(boss.damage * 1.0)  # Tăng từ 0.8 lên 1.0 (damage chuẩn)
            boss.speed = boss.speed * 1.0  # Tăng từ 0.9 lên 1.0 (tốc độ chuẩn)
    ...
    def spawn_next_boss(self):
        ...
        if self.current_boss_index < len(self.bosses):
            ...
            print(f"[MAP_MUA_THU_1] Spawned boss {self.current_boss_index}/{len(self.bosses)}")
        else:
            self.current_boss = None
            ...
            if not self.normal_enemies and not self.all_enemies_defeated:
                ...
                print("[MAP_MUA_THU_1] All enemies defeated! Moving to next stage...")
        ...
        if self.player.hp > 0:
            ...
            for enemy in self.normal_enemies:
                ...
                if self.player.state in ["danh", "da"] and self.player.actioning and not enemy.damaged:
                    ...
                    # If hitbox overlaps the enemy, apply damage on the final frames of the animation
            ...
            if current_time - self.death_timer > 2000:
                ...
                self.game.game_over_scene = self.game.load_scene("game_over", "Mùa Thu - Màn 1", score)
    ...
    def draw(self, screen):
        ...
        text = self.font.render("Mùa Thu - Màn 1: Rừng Vàng Nhẹ Nhàng", True, (184, 134, 11))
