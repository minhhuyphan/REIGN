...
class MapMuaThuMan2Scene(BaseMapScene):
    def __init__(self, game, player=None):
        ...
        load_and_apply_equipment(self.player, self.game, "MAP_MUA_THU_2")
    
        # MÀN 2: TRUNG BÌNH - NHIỀU QUÁI HỠN
        ...
        for group in range(4):
            ...
            for i in range(num_enemies):
                ...
                enemy = QuaiVat(x_pos, 300, folder_qv, sound_qv, color=(255, 140, 0), damage=18)  # Tăng từ 12 lên 18
                # Thiết lập vùng hoạt động - tăng sức mạnh
                enemy.hp = int(enemy.hp * 1.2)  # Tăng 20% HP
                enemy.speed = 2.8  # Tăng từ 2.5 lên 2.8
                enemy.home_x = x_pos
                enemy.patrol_range = 250  # Tăng từ 200 lên 250
                enemy.aggro_range = 400  # Tăng từ 300 lên 400
        ...
        for boss in self.bosses:
            boss.hp = int(boss.hp * 1.3)  # Tăng từ 1.0 lên 1.3 (30% máu hơn)
            boss.damage = int(boss.damage * 1.2)  # Tăng từ 1.0 lên 1.2 (20% sát thương hơn)
            boss.speed = boss.speed * 1.1  # Tăng từ 1.0 lên 1.1 (10% tốc độ hơn)
    ...
    def spawn_next_boss(self):
        ...
        if self.current_boss_index < len(self.bosses):
            ...
            print(f"[MAP_MUA_THU_MAN2] Spawned boss {self.current_boss_index}/{len(self.bosses)}")
        else:
            self.current_boss = None
            ...
            if not self.normal_enemies and not self.all_enemies_defeated:
                ...
                print("[MAP_MUA_THU_MAN2] All enemies defeated! Moving to next stage...")
    ...
    def update(self):
        ...
        if self.player.hp > 0:
            ...
            if current_time - self.death_timer > 2000:
                ...
                self.game.game_over_scene = self.game.load_scene("game_over", "Mùa Thu - Màn 2", score)
    ...
    def draw(self, screen):
        ...
        text = self.font.render("Mùa Thu - Màn 2: Gió Thu Dâng Trào", True, (139, 69, 19))
