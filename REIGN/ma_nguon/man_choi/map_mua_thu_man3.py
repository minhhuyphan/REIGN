...
class MapMuaThuMan3Scene:
    def __init__(self, game, player=None):
        ...
        if player:
            ...
            print(f"[MapMuaThuMan3] Received player with stats: HP={self.player.hp}, DMG={self.player.damage}, SPD={self.player.speed}")
        else:
            # Code tạo player mới
            ...
            print(f"[MapMuaThuMan3] Created new player with default stats")
        ...
        for i in range(180):
            self.falling_leaves.append({
                ...
                'speed': random.uniform(2.0, 5.0),  # Nhanh nhất
                'swing': random.uniform(1.0, 3.0),  # Dao động mạnh
                'swing_offset': random.uniform(0, 2 * math.pi),
                'size': random.randint(4, 10),  # Lá to nhất
    ...
    def update(self):
        ...
        if self.player.hp > 0:
            ...
            if current_time - self.death_timer > 2000:
                ...
                self.game.game_over_scene = self.game.load_scene("game_over", "Mùa Thu - Màn 3", score)
